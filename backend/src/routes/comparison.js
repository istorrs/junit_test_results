...
router.get('/runs', async (req, res) => {
  try {
    ...
    const cases1Map = new Map(
      cases1.map(c => [`${c.class_name}.${c.test_name}`, c])
    )
    const cases2Map = new Map(
      cases2.map(c => [`${c.class_name}.${c.test_name}`, c])
    )
    ...
    allTestIds.forEach(testId => {
      ...
      const testInfo = {
        ...
        time_diff_percent: case1.time > 0
      ...
      // Still failing
      else if ((status1 === 'failure' || status1 === 'error') &&
    ...
    const comparison = {
      run1: {
        ...
        time: testRun1.time,
        pass_rate: testRun1.tests > 0
          ? ((testRun1.tests - testRun1.failures - testRun1.errors - testRun1.skipped) / testRun1.tests) * 100
          : 0,
      ...
      run2: {
        ...
        time: testRun2.time,
        pass_rate: testRun2.tests > 0
          ? ((testRun2.tests - testRun2.failures - testRun2.errors - testRun2.skipped) / testRun2.tests) * 100
          : 0,
...
router.get('/test/:testId', async (req, res) => {
  try {
    ...
    const failedCount = history.filter(h => h.status === 'failure' || h.status === 'error').length
