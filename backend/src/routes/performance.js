...
router.get('/slowest', async (req, res) => {
  try {
    ...
    res.json({ slowest_tests: slowestTests })
  } catch (error) {
        ...
        $project: {
          ...
          percent_increase: {
            $round: [
              {
                $multiply: [
                  { $divide: [{ $subtract: ['$recent_avg', '$baseline_avg'] }, '$baseline_avg'] },
                  100
                ]
              },
              1
...
router.get('/test/:testId', async (req, res) => {
  try {
    ...
    if (testHistory.length === 0) {
      return res.status(404).json({ error: 'Test not found or no performance data available' })
    }
