...
router.get('/', async (req, res) => {
  try {
        ...
        $project: {
          ...
          pass_rate: {
            $multiply: [
              {
                $divide: [
                  { $subtract: ['$total_tests', { $add: ['$total_failures', '$total_errors'] }] },
                  '$total_tests'
                ]
              },
              100
    ...
    // Get total count
    const totalCount = await TestRun.distinct('release_tag').then(tags =>
      tags.filter(tag => tag != null).length
    )
  ...
  try {
    ...
    const comparison = {
      ...
      diff: {
        ...
        time_change_percent: metrics1.avg_time_per_run > 0
          ? ((metrics2.avg_time_per_run - metrics1.avg_time_per_run) / metrics1.avg_time_per_run) * 100
          : 0
      }
    }
